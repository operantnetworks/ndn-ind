...
static uint8_t TEST_RSA_PRIVATE_KEY_DER[] = {
 ...
 * private key.
 */
class CustomTpmKeyHandle : public TpmKeyHandle
{
public:
  /**
   * Create a CustomTpmKeyHandle to use the given in-memory private key.
   * @param key The in-memory private key.
   */
  CustomTpmKeyHandle(const ptr_lib::shared_ptr<TpmPrivateKey>& key)
  : key_(key) {}
 ...
 * the TPM back end.
 */
class CustomTpmBackEnd : public TpmBackEnd {
public:
  /**
   * Create a CustomTpmBackEnd and load the one in-memory private key with the
   * given encoded private key.
   * @param keyName The name of the key.
   * @param pkcs8 The byte array holding the encoded private key. The
   * encoding is an unencrypted PKCS #8 PrivateKeyInfo.
   * @param pkcs8Length The length of the encoding byte array.
   */
  CustomTpmBackEnd(const Name& keyName, const uint8_t* pkcs8, size_t pkcs8Length)
  : keyName_(keyName), key_(new TpmPrivateKey())
  {
    key_->loadPkcs8(pkcs8, pkcs8Length);
  }
...
private:
  ...
  {
    return keyName.equals(keyName_);
  }
  ...
  {
    if (!keyName.equals(keyName_))
      return ptr_lib::shared_ptr<TpmKeyHandle>();
    return ptr_lib::make_shared<CustomTpmKeyHandle>(key_);
  }

  virtual ptr_lib::shared_ptr<TpmKeyHandle>
  doCreateKey(const Name& identityName, const KeyParams& params)
  {
    throw runtime_error("CustomTpmBackEnd doCreateKey is not implemented");
  }

  virtual void
  doDeleteKey(const Name& keyName)
  {
    throw runtime_error("CustomTpmBackEnd doDeleteKey is not implemented");
  }
...
int main(int argc, char** argv)
{
  ...
  auto pib = ptr_lib::make_shared<PibMemory>();
  CertificateV2 certificate;
  certificate.wireDecode(TEST_CERTIFICATE, sizeof(TEST_CERTIFICATE));
  pib->addCertificate(certificate);

  // Create the custom TPM with the private key.
  auto tpm = ptr_lib::make_shared<CustomTpmBackEnd>
    (certificate.getKeyName(),
     TEST_RSA_PRIVATE_KEY_DER, sizeof(TEST_RSA_PRIVATE_KEY_DER));

  // Create a KeyChain with the PIB and the custom TPM.
  KeyChain keyChain(pib, tpm);

  // Create and sign a Data packet.
  Data data(Name("/ndn/abc"));
  data.setContent(Blob::fromRawStr("test content"));
  keyChain.sign(data);
  cout << "Signed Data: " << data.getName().toUri() << endl;
  ...
  validator.validate
    (data,
     [](auto&) { cout << "Data signature VERIFIED" << endl; },
     [](auto&, auto& error) {
       cout << "Data signature validate FAILED: " << error << endl;
     });
