 ...
 *
 * Original file: examples/test-full-psync-with-users.cpp
...
public:
    ...
    delayRangeMaxMs_(6000.0)
  {
    ...
    for (int i = 0; i < nUserPrefixes_; ++i) {
...
private:
  ...
  void
  processSyncUpdate
    (const ptr_lib::shared_ptr<std::vector<ptr_lib::shared_ptr<PSyncMissingDataInfo>>>& updates)
  {
    for (vector<ptr_lib::shared_ptr<PSyncMissingDataInfo>>::const_iterator it =
           updates->begin(); it != updates->end(); ++it) {
      const PSyncMissingDataInfo& update = **it;

      for (int i = update.lowSequenceNo_; i <= update.highSequenceNo_; i++)
        cout << "Update " << update.prefix_ << "/" << i << endl;
    }
  }
