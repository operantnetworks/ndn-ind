 ...
 *
 * Original file: examples/test-full-psync.cpp
...
/**
 * This generates new names with the given prefix and publishes them using
 * FullPSync2017. The also receives and displays new names from the other users.
 * This requires a running local NFD. This is compatible with the PSync
...
public:
  Producer
    (Face& face, KeyChain& keyChain, const Name& syncPrefix,
     const string& userPrefixUri, int nDataPrefixes, int maxNPublished)
  : face_(face),
    fullPSync_
      (80, face_, syncPrefix,
    ...
    delayRangeMaxMs_(6000.0)
  {
    ...
    for (int i = 0; i < nDataPrefixes_; ++i) {
      ostringstream prefixUri;
      prefixUri << userPrefixUri << "-" << i;
      Name dataPrefix(prefixUri.str());
...
private:
  void
  doUpdate(const Name& dataPrefix)
  {
    Name name = Name(dataPrefix).appendVersion((uint64_t)ndn_getNowMilliseconds());

    cout << "Publish: " << name << endl;
    fullPSync_.publishName(name);

    ++nPublished_[dataPrefix];
  ...
  void
  processSyncUpdate
    (const ptr_lib::shared_ptr<vector<Name>>& names)
  {
...
int main(int argc, char** argv)
{
  ...
  if (userPrefixUri == "") {
    cout << "You must enter a user prefix" << endl;
    return 1;
  }

  string syncPrefixUri = "/sync";
  int nDataPrefixes = 2;
  int maxNPublished = 3;
  ...
  try {
    ...
    Producer producer
      (face, keyChain, Name(syncPrefixUri), userPrefixUri, nDataPrefixes,
       maxNPublished);
