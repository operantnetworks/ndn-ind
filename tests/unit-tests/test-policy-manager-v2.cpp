...
static uint8_t TEST_RSA_PRIVATE_KEY_PKCS8[] = {
...
public:
  TestConfigPolicyManager()
  {
    ...
    pibImpl_.reset(new PibMemory());
    tpmBackEnd_.reset(new TpmBackEndMemory());
    policyManager_.reset(new ConfigPolicyManager
      (policyConfigDirectory_ + "/simple_rules.conf", 
       ptr_lib::make_shared<CertificateCacheV2>()));

    identityName_ = Name("/TestConfigPolicyManager/temp");
    // To match the anchor cert.
    keyName_ = Name(identityName_).append("KEY").append("ksk-1416010123");
    pibImpl_->addKey
      (identityName_, keyName_, TEST_RSA_PUBLIC_KEY_DER,
       sizeof(TEST_RSA_PUBLIC_KEY_DER));
    // Set the password null since we have an unencrypted PKCS #8 private key.
    tpmBackEnd_->importKey
      (keyName_, TEST_RSA_PRIVATE_KEY_PKCS8,
       sizeof(TEST_RSA_PRIVATE_KEY_PKCS8), 0, 0);

    keyChain_.reset(new KeyChain(pibImpl_, tpmBackEnd_, policyManager_));

    ptr_lib::shared_ptr<PibKey> pibKey = keyChain_->getPib().getIdentity
  ...
  Name identityName_;
  Name keyName_;
  ptr_lib::shared_ptr<PibMemory> pibImpl_;
