...
public:
  ...
  void onVerifiedInterest(const ptr_lib::shared_ptr<Interest>& interest)
  {
    ++successCount_;
  }
...
class TestConfigPolicyManager : public ::testing::Test {
public:
  TestConfigPolicyManager()
  {
    ...
    policyManager_.reset(new ConfigPolicyManager
      (policyConfigDirectory_ + "/simple_rules.conf"));

    identityName_ = Name("/TestConfigPolicyManager/temp");
    // To match the anchor cert.
    Name keyName = Name(identityName_).append("ksk-1416010123");
    privateKeyStorage_->setKeyPairForKeyName
      (keyName, KEY_TYPE_RSA, TEST_RSA_PUBLIC_KEY_DER,
       sizeof(TEST_RSA_PUBLIC_KEY_DER), TEST_RSA_PRIVATE_KEY_DER,
       sizeof(TEST_RSA_PRIVATE_KEY_DER));
    identityStorage_->addKey
      (keyName, KEY_TYPE_RSA, Blob(TEST_RSA_PUBLIC_KEY_DER,
       sizeof(TEST_RSA_PUBLIC_KEY_DER)));

    ptr_lib::shared_ptr<IdentityCertificate> cert =
      identityManager_->selfSign(keyName);
...
TEST_F(TestConfigPolicyManager, NoVerify)
{
  ptr_lib::shared_ptr<NoVerifyPolicyManager> policyManager
...
TEST_F(TestConfigPolicyManager, SelfVerification)
{
  ptr_lib::shared_ptr<SelfVerifyPolicyManager> policyManager
    (new SelfVerifyPolicyManager(identityStorage_.get()));
  KeyChain keyChain(identityManager_, policyManager);
  ...
  ptr_lib::shared_ptr<Data> data2(new Data(Name("/TestData/2")));
