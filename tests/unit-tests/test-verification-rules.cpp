...
public:
  ...
  static Name
  certNameFromKeyName(const Name& keyName, int keyIdx = -1)
  {
    if (keyIdx < 0)
...
TEST_F(TestVerificationRules, SimpleRegex)
{
  ...
  ASSERT_TRUE(matchedRule3);
  ASSERT_NE(matchedRule3, matchedRule1) <<
    "Rule regex matched extra components";
  ASSERT_TRUE(matchedRule4);
  ASSERT_NE(matchedRule4, matchedRule1) <<
